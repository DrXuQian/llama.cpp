...
// seqused_k is a device [batch] i32 (or null): the number of leading K/V entries that actually hold tokens. ggml's
// KV cache view is padded up so the graph shape stays reusable, so seqlen_k here is an upper bound; FA reads the
// causal offset off seqlen_k - seqlen_q, which without seqused_k lets a query row reach into the later tokens of
// its own ubatch. Null keeps seqlen_k at face value.
