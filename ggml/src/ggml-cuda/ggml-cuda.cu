...
static ggml_cuda_device_info ggml_cuda_init() {
    ...
    for (int id = 0; id < info.device_count; ++id) {
        ...
        // Avoid scheduler wakeup delays between short decode graphs.
        {
            ...
            CUDA_CHECK(cudaSetDeviceFlags((flags & ~cudaDeviceScheduleMask) | cudaDeviceScheduleSpin));
            GGML_LOG_INFO("%s: device %d uses spin scheduling\n", __func__, id);
        }
...
ggml_backend_cuda_context::~ggml_backend_cuda_context() {
    ...
    for (auto & input : input_buffers) {
        CUDA_CHECK(cudaEventSynchronize(input.copied));
        CUDA_CHECK(cudaEventDestroy(input.copied));
        CUDA_CHECK(cudaFreeHost(input.data));
    }
    if (input_stream) {
        CUDA_CHECK(cudaStreamDestroy(input_stream));
    }
#endif
...
struct ggml_cuda_copy_sources {
    const int32_t * src[12];
    int32_t * dst[12];
    int n[12];
};

static __global__ void ggml_cuda_copy_small_tensors(ggml_cuda_copy_sources copies) {
    const int i = blockIdx.x*blockDim.x + threadIdx.x;
    const int tensor = blockIdx.y;
    if (i < copies.n[tensor]) {
        copies.dst[tensor][i] = copies.src[tensor][i];
    }
}

static bool ggml_backend_cuda_get_tensors_async(ggml_backend_t backend, const ggml_tensor ** tensors, void ** dst, size_t count) {
    auto * cuda_ctx = (ggml_backend_cuda_context *) backend->context;
#if CUDART_VERSION >= 13000
    std::vector<const void *> dsts, srcs;
#else
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (buffer->buft != ggml_backend_cuda_buffer_type(cuda_ctx->device)) {
            return false;
        }
        const size_t size = ggml_nbytes(tensor);
        if (size > 0) {
            srcs.push_back(tensor->data);
    ...
    if (tokens) {
        ...
        CUDA_CHECK(cudaPointerGetAttributes(&attrs, dst[0]));
        if (attrs.type == cudaMemoryTypeDevice && attrs.device == ggml_cuda_get_physical_device(cuda_ctx->device)) {
            ggml_cuda_token_sources sources = {};
            for (size_t i = 0; i < count; i++) {
                sources.data[i] = (const int32_t *) tensors[i]->data;
            }
            // Pack scalar outputs in one operation before the consumer stream starts.
            ggml_cuda_gather_tokens<<<1, 32, 0, cuda_ctx->stream()>>>(sources, (int32_t *) dst[0], count);
            CUDA_CHECK(cudaGetLastError());
            return true;
        }
    }
    if (count >= 2 && count <= 12 && sizes.size() == count) {
        size_t total = 0;
        size_t max_size = 0;
        bool small = true;
        for (const size_t size : sizes) {
            ...
            small &= size % sizeof(int32_t) == 0;
        }
        small &= total <= 256*1024;
        for (size_t i = 0; small && i < count; ++i) {
            cudaPointerAttributes attrs;
            CUDA_CHECK(cudaPointerGetAttributes(&attrs, dst[i]));
            small = attrs.type == cudaMemoryTypeDevice && attrs.device == ggml_cuda_get_physical_device(cuda_ctx->device) &&
                (uintptr_t) srcs[i] % sizeof(int32_t) == 0 && (uintptr_t) dsts[i] % sizeof(int32_t) == 0;
        }
        if (small) {
            ggml_cuda_copy_sources copies = {};
            for (size_t i = 0; i < count; ++i) {
                copies.src[i] = (const int32_t *) srcs[i];
                copies.dst[i] = (int32_t *) dsts[i];
                copies.n[i] = sizes[i]/sizeof(int32_t);
            }
            const dim3 blocks((max_size/sizeof(int32_t) + 127)/128, count);
            ggml_cuda_copy_small_tensors<<<blocks, 128, 0, cuda_ctx->stream()>>>(copies);
            CUDA_CHECK(cudaGetLastError());
            return true;
        }
    }
    ...
    size_t attrs_idx = 0;
#if CUDART_VERSION >= 13000
    CUDA_CHECK(cudaMemcpyBatchAsync(dsts.data(), srcs.data(), sizes.data(), sizes.size(), &attrs, &attrs_idx, 1, cuda_ctx->stream()));
#else
    size_t fail_idx = SIZE_MAX;
    CUDA_CHECK(cudaMemcpyBatchAsync(dsts.data(), srcs.data(), sizes.data(), sizes.size(), &attrs, &attrs_idx, 1, &fail_idx, cuda_ctx->stream()));
#endif
    return true;
}

static bool ggml_backend_cuda_set_inputs_async(ggml_backend_t backend, ggml_tensor ** dst, ggml_tensor ** src, size_t count, ggml_backend_event_t consumed) {
    auto * cuda_ctx = (ggml_backend_cuda_context *) backend->context;
#if CUDART_VERSION >= 13000
    std::vector<const void *> dsts, srcs;
#else
    ...
    ggml_cuda_set_device(cuda_ctx->device);
    ggml_cuda_input_buffer * staging = nullptr;
    for (auto & input : cuda_ctx->input_buffers) {
        const cudaError_t status = cudaEventQuery(input.copied);
    ...
    if (!staging) {
        if (cuda_ctx->input_buffers.size() >= 8) {
            return false;
        }
        void * data = ggml_cuda_host_malloc(1024*1024);
        if (!data) {
            return false;
        }
        cudaEvent_t copied;
        CUDA_CHECK(cudaEventCreateWithFlags(&copied, cudaEventDisableTiming));
        cuda_ctx->input_buffers.push_back({ data, copied });
        staging = &cuda_ctx->input_buffers.back();
    }
    // Keep pinned sources until their stream-ordered transfers have completed.
    size_t offset = 0;
    for (size_t i = 0; i < count; i++) {
        const size_t size = ggml_nbytes(src[i]);
        GGML_ASSERT(size == ggml_nbytes(dst[i]));
        if (size == 0) {
            continue;
        }
        dsts.push_back(dst[i]->data);
        memcpy((char *) staging->data + offset, src[i]->data, size);
        srcs.push_back((char *) staging->data + offset);
        sizes.push_back(size);
        offset += size;
    }
    if (sizes.empty()) {
    ...
    if (consumed) {
        if (!cuda_ctx->input_stream) {
            CUDA_CHECK(cudaStreamCreateWithFlags(&cuda_ctx->input_stream, cudaStreamNonBlocking));
        }
        stream = cuda_ctx->input_stream;
        CUDA_CHECK(cudaStreamWaitEvent(stream, (cudaEvent_t) consumed->context, 0));
    }
    cudaMemcpyAttributes attrs = {};
    attrs.srcAccessOrder = cudaMemcpySrcAccessOrderStream;
    size_t attrs_idx = 0;
#if CUDART_VERSION >= 13000
    CUDA_CHECK(cudaMemcpyBatchAsync(dsts.data(), srcs.data(), sizes.data(), sizes.size(), &attrs, &attrs_idx, 1, stream));
#else
    size_t fail_idx = SIZE_MAX;
    CUDA_CHECK(cudaMemcpyBatchAsync(dsts.data(), srcs.data(), sizes.data(), sizes.size(), &attrs, &attrs_idx, 1, &fail_idx, stream));
#endif
    CUDA_CHECK(cudaEventRecord(staging->copied, stream));
    if (consumed) {
        CUDA_CHECK(cudaStreamWaitEvent(cuda_ctx->stream(), staging->copied, 0));
    }
    return true;
}

static bool ggml_backend_cuda_set_inputs(ggml_backend_t backend, ggml_tensor ** dst, ggml_tensor ** src, size_t count) {
    return ggml_backend_cuda_set_inputs_async(backend, dst, src, count, nullptr);
}
#endif
...
static void ggml_cuda_graph_evaluate_and_capture(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, const bool use_cuda_graph, const bool cuda_graph_update_required, const ggml_cuda_graph_key & graph_key, const bool launch = true) {
...
static bool ggml_backend_cuda_graph_prepare(ggml_backend_t backend, ggml_cgraph * cgraph) {
    auto * cuda_ctx = (ggml_backend_cuda_context *) backend->context;
    ggml_cuda_set_device(cuda_ctx->device);
    if (cgraph->n_nodes == 0) {
        return false;
    ...
    const auto graph_key = ggml_cuda_graph_get_key(cgraph);
    if (!ggml_cuda_graph_set_enabled(cuda_ctx, graph_key) || !ggml_cuda_graph_check_compability(cgraph)) {
        return false;
    }

#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA) && CUDART_VERSION >= 12080
    ggml_cuda_graph_prepare_early(cuda_ctx, cgraph);
#endif
    auto * graph = cuda_ctx->cuda_graph(graph_key);
    const bool changed = ggml_cuda_graph_update_required(cuda_ctx, cgraph);
    if (!changed && graph->instance && graph->warmup_complete) {
        return true;
    }
    ...
    ggml_cuda_graph_evaluate_and_capture(cuda_ctx, cgraph, true, true, graph_key, false);
...
static void ggml_cuda_graph_capture_begin(ggml_backend_cuda_context * ctx, cudaGraph_t body,
    ...
    CUDA_CHECK(cudaStreamBeginCaptureToGraph(ctx->stream(), body, dependencies.data(), nullptr,
            dependencies.size(), cudaStreamCaptureModeRelaxed));
}

static std::vector<cudaGraphNode_t> ggml_cuda_graph_capture_end(ggml_backend_cuda_context * ctx) {
    cudaStreamCaptureStatus status;
    cudaGraph_t body;
    const cudaGraphNode_t * dependencies;
    size_t n_dependencies;
    CUDA_CHECK(cudaStreamGetCaptureInfo(ctx->stream(), &status, nullptr, &body, &dependencies, &n_dependencies));
    std::vector<cudaGraphNode_t> result(dependencies, dependencies + n_dependencies);
    CUDA_CHECK(cudaStreamEndCapture(ctx->stream(), &body));
    {
        std::lock_guard<std::mutex> lock(ggml_cuda_lock);
        if (ggml_cuda_lock_counter.fetch_sub(1, std::memory_order_relaxed) == 1) {
            ggml_cuda_lock_cv.notify_all();
...
static void ggml_cuda_graph_capture_early(ggml_backend_cuda_context * ctx, ggml_cgraph * cgraph,
        cudaGraph_t root, cudaGraph_t parent, const ggml_cuda_graph & config, const ggml_cuda_graph_key & key,
        std::vector<cudaGraphNode_t> & dependencies) {
    const int n_steps = config.early_ends.size();
    std::vector<cudaGraphConditionalHandle> handles(n_steps - 1);
    ...
    for (int step = 0; step < n_steps; ++step) {
        ...
        if (step == 0) {
            ...
            CUDA_CHECK(cudaGraphAddNode(&node, parent, dependencies.data(), dependencies.size(), &params));
        ...
        if (step == 0) {
            CUDA_CHECK(cudaGraphAddChildGraphNode(&node, parent, dependencies.data(), dependencies.size(), body));
...
static bool ggml_backend_cuda_graph_early_exit(ggml_backend_t backend, ggml_cgraph * cgraph,
    ...
    ends.back() = cgraph->n_nodes;
    graph->early_ends = std::move(ends);
    graph->early_scores.assign(scores, scores + n_steps);
    graph->early_p_min = p_min;
    graph->early_counts = counts->data;
    graph->early_uid = cgraph->uid;
...
static void ggml_cuda_graph_prepare_early(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph) {
    ...
    std::vector<cudaGraphNode_t> dependencies;
    ggml_cuda_graph_capture_early(cuda_ctx, cgraph, graph->graph, graph->graph, *graph, key, dependencies);
    CUDA_CHECK(cudaGraphInstantiate(&graph->instance, graph->graph, 0));
    ggml_cuda_graph_update_required(cuda_ctx, cgraph);
    graph->early_instance_revision = graph->early_revision;
    graph->warmup_complete = true;
}
static __global__ void ggml_cuda_graph_select(const int32_t * selector, cudaGraphConditionalHandle handle) {
...
static bool ggml_backend_cuda_graph_select(ggml_backend_t backend, ggml_cgraph * primary,
    ...
    auto * graph = cuda_ctx->cuda_graph(key);
    int branch_stages = n_stages;
    while (branch_stages > 0) {
        bool shared = true;
        for (int i = 1; i < n_graphs; ++i) {
            shared &= stages[i*n_stages + branch_stages - 1] == stages[branch_stages - 1];
        }
        if (!shared) {
            break;
        }
    ...
    bool nested_early = false;
    std::vector<uint64_t> uids = { (uint64_t) n_graphs, (uint64_t) n_stages };
    for (int i = 0; i < n_graphs*n_stages; ++i) {
        auto * stage = stages[i];
        auto * config = stage ? cuda_ctx->cuda_graph(ggml_cuda_graph_get_key(stage)) : nullptr;
    ...
    const auto previous_instance = graph->instance;
    const auto previous_graph = graph->graph;
    const bool previous_nested = !graph->select_uids.empty() && graph->select_uids.back();
    graph->instance = nullptr;
    CUDA_CHECK(cudaGraphCreate(&graph->graph, 0));
    ...
    auto capture = [&](cudaGraph_t body, int i, int begin, int end, std::vector<cudaGraphNode_t> dependencies) {
        for (int j = begin; j < end; ++j) {
            if (auto * stage = stages[i*n_stages + j]) {
                ...
                if (config->early_uid == stage->uid && config->early_p_min > 0.0f) {
                    ...
                    dependencies = ggml_cuda_graph_capture_end(cuda_ctx);
                }
            }
        }
    };
    for (int i = 0; i < n_graphs; ++i) {
        capture(selector ? params.conditional.phGraph_out[i] : graph->graph, i, 0, branch_stages, {});
    }
    capture(graph->graph, 0, branch_stages, n_stages, branch ? std::vector<cudaGraphNode_t>{ branch } : std::vector<cudaGraphNode_t>{});
    ...
    if (previous_instance) {
        // Nested conditional handles need a fresh executable when the composition changes.
        bool recreate = nested_early || previous_nested;
        if (!recreate) {
            cudaGraphExecUpdateResultInfo info;
            const auto status = cudaGraphExecUpdate(graph->instance, graph->graph, &info);
    ...
    if (previous_graph) {
        CUDA_CHECK(cudaGraphDestroy(previous_graph));
    }
    if (!graph->instance) {
        CUDA_CHECK(cudaGraphInstantiate(&graph->instance, graph->graph, 0));
    }
...
static enum ggml_status ggml_backend_cuda_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    ggml_cuda_graph * graph = cuda_ctx->cuda_graph(graph_key);
    if (graph->is_enabled()) {
        // A scheduler UID is stable only while the graph and its allocations are unchanged.
        // Replaying an already captured graph needs no per-node compatibility checks.
        if (cgraph->uid != 0 && cgraph->uid == graph->uid &&
            graph->warmup_complete && graph->instance != nullptr) {
            GGML_ASSERT((int) graph->node_props.size() == cgraph->n_nodes);
            CUDA_CHECK(cudaGraphLaunch(graph->instance, cuda_ctx->stream()));
            return GGML_STATUS_SUCCESS;
        }
    ...
    if (use_cuda_graph && cuda_graph_update_required) {
        // Drain uncaptured input transfers before starting a new capture.
        CUDA_CHECK(cudaStreamSynchronize(cuda_ctx->stream()));
        ...
        case GGML_OP_CPY:
            {
                ...
                if (src0_type == GGML_TYPE_F32 && src1_type == GGML_TYPE_I64) {
                    return true;
                }
...
static void * ggml_backend_cuda_reg_get_proc_address(ggml_backend_reg_t reg, const char * name) {
    ...
    if (strcmp(name, "ggml_backend_graph_prepare") == 0) {
        return (void *) ggml_backend_cuda_graph_prepare;
    }
#endif
#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA) && CUDART_VERSION >= 12080
    if (strcmp(name, "ggml_backend_graph_early_exit") == 0) {
#ifdef USE_CUDA_GRAPH
        return (void *) ggml_backend_cuda_graph_early_exit;
#else
        return nullptr;
#endif
    }
    if (strcmp(name, "ggml_backend_graph_select") == 0) {
#if defined(USE_CUDA_GRAPH) && !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA) && CUDART_VERSION >= 12080
        return (void *) ggml_backend_cuda_graph_select;
#endif
    }
    if (strcmp(name, "ggml_backend_set_inputs") == 0) {
