...
static void ncp_fa_init(void) {
    ...
    // NOTE: the seqused_k argument was added to _v3 WITHOUT a new name, so the symbol no longer guards against a
    // mismatched .so -- the .so and llama.cpp must be rebuilt together or the .so misreads the argument list.
    if (fa) {
...
extern "C" int ggml_ncp_lib_flash_attn_fwd(
        ...
        float scale, float softcap, int is_causal, int wl, int wr, const int * seqused_k, int dtype, void * stream) {
    ensure_fa_init();
    if (!g_fa_fn) return -1;
    return g_fa_fn(q, k, v, o, batch, seqlen_q, seqlen_k, n_heads_q, n_heads_kv, head_dim, head_dim_v,
                   ...
                   scale, softcap, is_causal, wl, wr, seqused_k, dtype, stream);
}
...
extern "C" int  ggml_ncp_lib_flash_attn_fwd(
        ...
        long long, long long, long long, long long, long long, long long,
        long long, long long, long long, long long, long long, long long,
        float, float, int, int, int, const int *, int, void *) { return -1; }
