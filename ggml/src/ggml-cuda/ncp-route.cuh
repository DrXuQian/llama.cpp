...
static void ggml_ncp_route_log(const ggml_tensor * dst, const char * kernel, const char * declined) {
    ...
    // A quantized src0 declining is not news: ggml_cuda_mul_mat_id dequantizes those weights to bf16 and offers the
    // node again a few lines later. Both calls name it after dst->src[0], which the dequant leaves alone, so tracing
    // the refusal would pair every real decision with a contradictory one under an identical name and shape -- and
    // spend two of the 64 dedup slots below where one belongs.
    if (declined && mmid && ggml_is_quantized(src0->type)) {
        return;
    }
