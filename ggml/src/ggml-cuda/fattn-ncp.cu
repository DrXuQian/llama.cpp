...
bool ggml_cuda_flash_attn_ext_ncp_lib(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // seqlen_k is K->ne[1], i.e. the KV cache view's cell count rounded up so ggml can reuse the graph -- an upper
    // bound on the history, not the history. FA has no mask input and reads the causal offset off
    // seqlen_k - seqlen_q, so on that bound a query row reaches j <= i + (pad - seqlen_q): past its own position,
    // into the later tokens of the same ubatch. seqused_k carries the live count so the kernel recovers the real
    // offset; it is a device pointer rather than a scalar because it changes per ubatch while a captured graph is
    // replayed.
    //
    // Required, not optional, whenever the mask is being replaced by the positional path: absent it we would be back
    // on the padded bound. The graph leaves it unset for every case where the count would not mean a contiguous
    // per-batch history -- a unified cache, or an attention input that does not carry one -- so "missing" and
    // "meaningless" are the same answer here, and both belong on the inline path.
    const ggml_tensor * kv_used = ggml_flash_attn_ext_get_kv_used(dst);
    if (is_causal && (!kv_used || !kv_used->data || kv_used->ne[0] < batch)) {
        return false;
    }
    const int * seqused_k = kv_used && kv_used->data ? (const int *) kv_used->data : nullptr;

    // seqused_k is dereferenced on device: a host pointer reads as garbage. Copied to CUDA0 today, not by contract.
    if (seqused_k && ggml_backend_buffer_is_host(kv_used->buffer)) {
        return false;
    }
