...
// Minimum N (= src0->ne[1], the per-expert output width) the .so's NoPad entry is trusted with. It computes wrong
// results for N = 50 and returns 0 while doing it, against the contract stated in ncp-moe-lib.h -- "non-zero if the
// .so has no kernel for this (N,K)" -- so the rc != 0 fallback in the hook is never reached for it and this has to
// stand in.
//
// 32 IS A BOUND, NOT A MEASUREMENT. N = 32, 64 and 512 are served correctly, so whatever the real requirement is it
// is at most 32; N = 50 is not, so it does not divide 50. Among the powers of two that leaves 4, 8, 16 and 32, and no
// test shape distinguishes them -- every bf16 mul_mat_id case in test-backend-ops has an N that is either a multiple
// of 32 or is 50. 32 is the only one of the four that cannot be too loose, and too loose here means handing the .so
// another shape it silently gets wrong rather than one it declines.
//
// Over-rejecting costs nothing worth measuring: N is an expert intermediate or hidden size, and the real ones are
// multiples of 32 (2880 for gpt-oss is the tightest, and it is NOT a multiple of 128 -- that is why this is 32).
// The fix belongs in the .so, either by refusing these shapes as documented or by exporting the requirement the way
// ncp_get_m_alignment_for_contiguous_layout already exports the M-direction one. Drop this then; until then do not
// move the constant without measuring, in either direction.
static constexpr int64_t NCP_MOE_N_ALIGN = 32;
...
static bool ggml_cuda_mul_mat_id_ncp_lib_impl(ggml_backend_cuda_context & ctx,
    ...
    // See NCP_MOE_N_ALIGN: the .so accepts unaligned N and computes it wrong, so refuse it here instead.
    if (N % NCP_MOE_N_ALIGN != 0) {
        return false;
    }
    ...
    // write_inverse=false keeps ids_src1 as the forward map (compact row -> src1 row), which is what the gather
    // below indexes with. true would give the inverse map (token slot -> compact row) that mmq's dedup path wants
    // when it quantizes each broadcast token once; we dequantize per compact row, so we need the forward map.
...
bool ggml_cuda_mul_mat_id_ncp_lib_supported(const ggml_tensor * tensor) {
    // Mirrors the gates ggml_cuda_mul_mat_id_ncp_lib_impl applies before it commits to anything, minus the scratch
    // and shared-memory limits: a caller asking this only needs to know whether the route is plausible, and
    // answering "yes" when the hook later declines costs one dropped fusion, not correctness. The N alignment is
    // mirrored even so -- unlike those two it is a static property of the weight tensor rather than of the device or
    // the ubatch, and leaving it out would cost an unaligned shape both the .so AND its GLU fusion, which is worse
    // than not having this hook at all.
    return tensor->op == GGML_OP_MUL_MAT_ID && ggml_ncp_lib_moe_nopad_available() &&
           ...
           tensor->src[0]->ne[1] % NCP_MOE_N_ALIGN == 0 &&
