...
struct common_sampler {
    ...
    void set_logits(struct llama_context * ctx, const llama_sampling_result & result) {
        const float *       sampled_probs  = result.probs;
        const float *       sampled_logits = result.logits;
        const llama_token * sampled_ids    = result.candidates;
        ...
        if (sampled_probs) {
            const uint32_t sampled_probs_count = result.n_probs;
            cur.resize(sampled_probs_count);
            for (uint32_t i = 0; i < sampled_probs_count; ++i) {
                cur[i] = llama_token_data{sampled_ids[i], sampled_logits[i], sampled_probs[i]};
            }
        } else if (sampled_logits) {
            const uint32_t sampled_logits_count = result.n_logits;
            cur.resize(sampled_logits_count);
            for (uint32_t i = 0; i < sampled_logits_count; i++) {
                cur[i] = llama_token_data{sampled_ids[i], sampled_logits[i], 0.0f};
            }
        } else {
            const auto * logits = result.raw_logits;
    ...
    llama_token backend_token = LLAMA_TOKEN_NULL;
};
...
llama_token common_sampler_sample(struct common_sampler * gsmpl, struct llama_context * ctx, int idx, bool grammar_first) {
    const auto sampling_result = llama_get_sampling_result(ctx, idx);
    ...
    gsmpl->set_logits(ctx, sampling_result);

    // Check if a backend sampler has already sampled a token in which case we
    // return that token id directly.
    {
        id = gsmpl->backend_token;
...
std::vector<llama_token> common_sampler_sample_and_accept_n(struct common_sampler * gsmpl, struct llama_context * ctx, const std::vector<int> & idxs, const llama_tokens & draft, bool grammar_first) {
    return common_sampler_sample_and_accept_n(gsmpl, ctx, idxs, draft, grammar_first, -1);
}
...
std::vector<llama_token> common_sampler_sample_and_accept_n(struct common_sampler * gsmpl, struct llama_context * ctx, const std::vector<int> & idxs, const llama_tokens & draft, bool grammar_first, int32_t max_tokens) {
    ...
    llama_token resolved[8];
    const llama_token * proposals = draft.data();
    bool tree_rows = draft.size() == 3 || draft.size() == 7;
    for (size_t i = 0; tree_rows && i < idxs.size(); ++i) {
        tree_rows = idxs[i] == (int) i;
    }
    if (tree_rows && llama_get_nextn_verified_draft(ctx, resolved, draft.size())) {
        proposals = resolved;
    }

    const size_t n_draft = max_tokens > 0 ? std::min(draft.size(), (size_t) max_tokens - 1) : draft.size();
