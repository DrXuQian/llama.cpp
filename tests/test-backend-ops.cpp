...
// member added upstream and appended to vars() silently stops all of them from matching. src_overlap prints
// through the std::to_string overload, so it is "0"/"1" and not "false"/"true".
const char * const test_mul_mat::skipped_cases[] = {
    "type_a=f16,type_b=f16,m=16,n=1,k=256,bs=[1,1],nr=[1,1],per=[0,1,2,3],k_v=0,o=1,src_overlap=0",
...
struct test_flash_attn_ext : public test_case {
    ...
    std::string vars() override {
        return VARS_TO_STR17(hsk, hsv, nh, nr23, kv, nb, mask, sinks, max_bias, logit_softcap, prec, type_K, type_V, permute, kv_view, v_is_view_of_k, causal);
    }
    ...
    test_flash_attn_ext(int64_t hsk = 128, int64_t hsv = 128, int64_t nh = 32, std::array<int64_t, 2> nr23 = {1, 1}, int64_t kv = 96, int64_t nb = 8,
                        ...
                        bool kv_view = true, bool v_is_view_of_k = false, bool causal = false)
        : hsk(hsk), hsv(hsv), nh(nh), nr23(nr23), kv(kv), nb(nb), mask(mask), sinks(sinks), max_bias(max_bias), logit_softcap(logit_softcap), prec(prec),
          type_K(type_K), type_V(type_V), permute(permute), kv_view(kv_view), v_is_view_of_k(v_is_view_of_k), causal(causal) {
                ...
                for (int kv : { 113, 512, 1024 }) {
                    ...
                    for (int nb : { 1, 3, 32, 75 }) {
                        test_cases.emplace_back(new test_flash_attn_ext(
                                    ...
                                    GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3},
                                    /*kv_view =*/ true, /*v_is_view_of_k =*/ false, /*causal =*/ true));
                        // run fewer test cases permuted
                        if (kv == 512) {
                            test_cases.emplace_back(new test_flash_attn_ext(
                                        hsk, hsv, 4, {nr2, nr3}, kv, nb, true, false, 0, 0, GGML_PREC_F32,
