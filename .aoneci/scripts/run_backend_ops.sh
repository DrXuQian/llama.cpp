...
# Write logs directly to the shared directory so partial logs survive interruption.
# Use a timestamp+PID subdirectory to avoid collisions between concurrent runs.
TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
RUN_ID="${TIMESTAMP}_$$"
OUTPUT_DIR="/ppusw/share/eec_shared/llama_backend_ops_${RUN_ID}"
mkdir -p "$OUTPUT_DIR"
...
# Package logs on failure; clean up raw directory on success.
# If the script was interrupted before this point, the raw output directory remains intact.
if [ "$EXIT_CODE" -ne 0 ]; then
    TARBALL="/ppusw/share/eec_shared/llama_backend_ops_logs_${RUN_ID}.tar.gz"
    tar -zcf "$TARBALL" -C /ppusw/share/eec_shared "llama_backend_ops_${RUN_ID}"
    rm -rf "$OUTPUT_DIR"
    echo "Failed test logs packaged to: $TARBALL"
else
    rm -rf "$OUTPUT_DIR"
    echo "Logs cleaned up: $OUTPUT_DIR"
fi
