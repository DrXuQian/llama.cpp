...
// ---------------------------------------------------------------------------------------------------------------
// MASKED layout. A = [G, max_m, K], out = [G, max_m, N], masked_m[g] = how many of group g's max_m rows are real.
//
// Why this and not the contiguous entry above: contiguous pins BLOCK_M to the 128-row alignment, so an expert
// holding 32 rows still costs a full 128-row wgmma -- 4x the useful FLOPs. Masked lets the scheduler skip whole
// row-blocks past masked_m[g] and its BLOCK_M candidates are {64, 128} (heuristics/sm90.hpp), so the same expert
// costs ceil(32/64)*64 = 64 rows. masked_m needs NO alignment at all (upstream's own test drives it with 20 rows
// per group). Cost: A/out are sized G*max_m rather than sum(padded m_g), which only loses on heavy imbalance.
//
// Rows [masked_m[g], max_m) of A are never read into a live output row, so the caller may leave them
// uninitialized -- garbage in row r only ever reaches out[g][r], which the caller drops.
//
// Torch-free re-implementation of sm90_bf16_m_grouped_gemm_masked (csrc/jit_kernels/impls/sm90_bf16_gemm.hpp:202).
// `expected_m` is the average real rows per expert; unlike the contiguous entry this one genuinely uses it (it
// steers the tile heuristic), so pass total_real_rows / n_experts, not the row count.
extern "C" int ppu_moe_grouped_gemm_bf16_masked(
    ...
    if (max_m <= 0 || N <= 0 || K <= 0 || n_experts <= 0) return 1;
    if (K % 64 != 0) return 1;                      // DG_HOST_ASSERT(k % 64 == 0) in the impl
    ...
    try {
        ...
        const int m = max_m, n = N, k = K, num_groups = n_experts;
        const auto major_a = cute::UMMA::Major::K;   // A [G,m,k], k-contiguous
        const auto major_b = cute::UMMA::Major::K;   // B [G,n,k], k-contiguous
        const std::string compiled_dims = "nk";
        ...
        const auto desc = GemmDesc {
            ...
            .expected_n = 0, .expected_k = 0,
            .expected_num_groups = num_groups
        };
        const auto config = get_best_config<SM90ArchSpec>(desc);

        // Same 2D descriptors as the contiguous entry, except A and D carry the group dim: upstream's
        // make_tma_a_desc/make_tma_cd_desc fold it in as `shape_m * num_groups` (runtime_utils.hpp:200,240), i.e.
        // [G,m,k] is just [G*m, k] with outer stride k. B was already grouped this way.
        const auto tensor_map_a = make_tma_2d_desc_bf16(const_cast<void *>(A),
            /*gmem*/ k, m * num_groups, /*smem*/ config.layout.block_k, config.storage_config.load_block_m,
            /*outer_stride*/ k, config.storage_config.swizzle_a_mode);
        const auto tensor_map_b = make_tma_2d_desc_bf16(const_cast<void *>(B),
            /*gmem*/ k, n * num_groups, /*smem*/ config.layout.block_k, config.storage_config.load_block_n,
            /*outer_stride*/ k, config.storage_config.swizzle_b_mode);
        const auto tensor_map_cd = make_tma_2d_desc_bf16(out,
            /*gmem*/ n, m * num_groups, /*smem*/ config.storage_config.store_block_n, config.storage_config.store_block_m,
            /*outer_stride*/ n, config.storage_config.swizzle_cd_mode);
        ...
        const SM90BF16GemmRuntime::Args args = {
            ...
            .launch_args = LaunchArgs(config.launch_config.num_sms, config.launch_config.num_threads,
                                      config.pipeline_config.smem_size,
                                      config.layout.get_cluster_size()),
            .grouped_layout = const_cast<int *>(masked_m),   // int32[G], NOT per-row
            .tensor_map_a = tensor_map_a,
            .tensor_map_b = tensor_map_b,
            .tensor_map_cd = tensor_map_cd,
        };
        ...
        fprintf(stderr, "[ppu-moe] masked grouped-gemm (max_m=%d N=%d K=%d G=%d) failed: %s -> inline fallback\n",
                max_m, N, K, n_experts, e.what());
        return 2;
    }
}
