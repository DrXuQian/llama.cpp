...
These remain private graph/tensor contracts. New algorithms should reuse execution and output ownership without changing the server decode sequence or introducing per-step CPU callbacks. Magic MTP uses these same graph bundles; tree execution for recurrent targets remains subsequent work.
...
## Random MTP and experimental Magic MTP

Eligible single-slot CUDA MTP requests enable backend sampling automatically for supported random samplers as well as greedy samplers. Ordinary random MTP uses a device window copied from the target sampler's current RNG state at bootstrap. Target positions select its uniforms inside the graph; accepted tokens retain the existing CPU RNG accounting. The next target can be submitted before CPU acceptance without uploading uniforms after each rejection. Sampler filtering stays in the existing backend sampler chain.

`--spec-magic-mtp` opts into the vLLM Ascend Magic MTP block verification algorithm. `--no-spec-magic-mtp` disables it; its default is off because it changes the output distribution. It requires the supported Qwen MTP GPU prefetch path, random target sampling, `--spec-draft-n-max` from 3 through 8 (K=4 through 9 including the root), `--spec-draft-p-min 0`, and target probability/candidate outputs. Greedy sampling and unsupported configurations retain ordinary verification. Cold bootstrap and fallback verification also remain ordinary sampling. This is the reference's point-mass proposal case: the existing greedy draft has q(draft)=1, not its softmax confidence.

For proposal i, let p_i be its probability after the target's sampling filters. The accepted count is the longest prefix j for which the product of p_1 through p_j is at least the product of j independent uniforms. An earlier failed prefix may therefore be included by a later successful prefix. Short scalar log sums avoid product underflow and the inf-inf intermediates of a parallel scan when a proposal has zero probability. Rejection samples from the target distribution with the draft ID removed; full acceptance uses the target's bonus sample. Existing GGML operators build acceptance and recovery in `llama-spec-sampling.h`. Effective sampled IDs feed the existing GPU acceptance, catch-up and recurrent-state commit path; no new GGML or CUDA operation is required.

Magic observes uniforms for the whole block, including a rejected suffix. It must not reuse those draws at subsequent token positions. A separate RNG stream therefore uses nine draws per GPU round, indexed by the existing device step counter. Recovery uses another separate stream. Both derive from the sampler RNG state at each bootstrap, with different stream IDs; ordinary sampler draws remain independent of these streams. Fixed allocation addresses survive compatible graph reuse. A new bootstrap, sampler seed or sampling mode rebases the window, including after graph recreation or state restoration. There is no steady CPU probability read or RNG upload. Bootstrap uploads and unsupported fallback work still have their normal cost.

The local reference is vLLM Ascend commit `eec0c99`, `vllm_ascend/sample/rejection_sampler.py`, function `rejection_random_sample_block_verify_pytorch`. Its greedy path returns before block verification. Both the Python and Triton block formulas agree for this point-mass proposal case; general probabilistic drafts need separate proposal-distribution and rejection validation. Entropy verification is not enabled here. This approximate method must be compared with ordinary MTP at the same temperature and filters, and acceptance gains must not be described as preserving the target distribution or as proving generation quality.

The activation log reports `GPU MTP random sampling active`, with position indexing for ordinary random MTP or round indexing and `magic_mtp=1` for Magic. `--no-spec-gpu-pipeline` retains the existing pipeline opt-out. No optimization environment variable is added.

On RTX 5090 / CUDA 12.8 with Qwen3.6-35B-A3B-UD-Q4_K_XL, temperature 0.8, one slot, context 2048 and K=4, three 512-token prompts repeated three times measured 396.7 token/s for ordinary random MTP and 429.8 token/s for Magic (+8.3%, total tokens divided by total decode time). These timings include request preparation inside decode; they are separate from profiling. In the Magic Nsight Systems trace, all 146 measured steady target handoffs were queued before the preceding target finished, with at least 5.8 ms of submission lead. Maximum steady idle was 3.8 us; ordinary dispatch gaps remain. The whole request still had a 28.1 ms cold preparation gap. Real-model Compute Sanitizer reported zero errors. These bounded measurements establish pipeline behavior, not general quality or speed guarantees.
...
Composed K=8 tree drafts use GPU upper-bound early skip by default; `--no-spec-eagle3-tree-early-skip` disables it for comparisons. An unseen descendant cannot exceed the current primary node's cumulative probability. Once seven seen candidates meet that bound, no later candidate can enter the selected eight rows. Equal scores favor the earlier IDs. This preserves the full-draft tree choice; it does not impose a confidence threshold or reduce the target-row budget. The K=4 bound was also validated, but its measured throughput benefit was inconsistent, so K=4 retains full drafting.

The existing conditional CUDA graph accepts I32 continuation predicates as well as the chain's F32 confidence scores. The remaining draft heads form nested conditional bodies so stopping bypasses the unused subtree. Its first decision resets the execution count on the GPU. Tree width remains fixed while the device control records the actual number of executed draft heads. Candidate, logit and score storage is cleared within the draft graph, so inactive rows remain placeholders until the verified path is resolved. CPU acceptance still consumes the resolved tree after submitting the next target. If the prepared tree cannot be consumed, `complete_draft()` disables the condition and materializes the full chain before ordinary verification or draft collection. This fallback must not consume inactive candidates from an earlier round.

Standalone startup drafts and unsupported settings retain full drafting. Existing confidence stopping remains available on the chain path through `--spec-draft-p-min`; combining that threshold with tree verification needs separate validation. Conditional dispatch has its own GPU cost and needs a throughput and timeline comparison.
