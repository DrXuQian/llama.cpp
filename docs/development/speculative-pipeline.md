...
The draft follows three primary tokens, retaining two candidates per step and a third candidate at the root. Probabilities are normalized over the full vocabulary. Original IDs are `[root, main1, side1, main2, side2, main3, side3, root3]`. The four target rows, including the root, can use `[0,1,2,3]`, `[0,1,3,4]`, `[0,1,3,5]`, or `[0,1,2,7]`. The last topology wins when the third root candidate has greater cumulative probability than `main2`; otherwise strict comparisons of IDs 2, 4 and 5 choose among the original templates. Ties preserve the lower original ID. These templates maximize the sum of draft path probabilities in this bounded comb; they do not search descendants of side candidates.
...
After generation has started, eligible single-slot greedy EAGLE3 tree requests retain the four-row verification budget near the output quota. The bounded sampler overload commits at most the remaining output count, including its final target-sampled token. If truncation changes the accepted length, that request has reached its quota; the existing rollback and release paths discard unused work. This avoids building shorter decode graphs at the request tail. Initial short requests, synthetic sampling, hybrid/recurrent targets and other unsupported settings retain the usual draft-length limits. This does not remove cold graph preparation or GPU kernel-dispatch gaps.
